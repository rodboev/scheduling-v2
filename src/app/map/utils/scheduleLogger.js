...
import { calculateDistance, calculateTravelTime } from '@/app/utils/distance'
...
function findGaps(services) {
  ...
  const sortedServices = [...services].sort((a, b) => 
    new Date(a.time.visited) - new Date(b.time.visited)
  )

  for (let i = 0; i < sortedServices.length - 1; i++) {
    const currentEnd = new Date(sortedServices[i].time.visited).getTime() + 
    ...
    if (gapDuration > 0.25) { // Only log gaps longer than 15 minutes
      gaps.push({
...
export function logSchedule(services) {
  ...
  if (hasDifferentReasons) {
    ...
    services
      .filter(s => s.schedulingReason)
      .forEach(s => {
        log += `- ${s.company}: ${s.schedulingReason}\n`
      })
    log += '\n'
  }

  // Process each original cluster
  for (const [clusterNum, clusterServices] of Object.entries(originalClusters)) {
    if (clusterNum === 'noise' || clusterNum === 'outliers' || 
        clusterNum === 'overlap' || clusterNum === 'outOfRange' || 
        clusterNum === 'other') continue

    // Sort services by visited time
    const sortedServices = [...clusterServices].sort((a, b) => 
      new Date(a.time.visited) - new Date(b.time.visited)
    )
    ...
    subClusters.forEach((subCluster, subIndex) => {
      ...
      const clusterEnd = new Date(lastService.time.visited).getTime() + 
      ...
      for (let i = 0; i < subCluster.length; i++) {
        ...
        const visitEnd = new Date(visitStart.getTime() + service.time.duration * 60000)
        ...
        if (i > 0) {
          ...
          const timeGap = (
            ...
            (new Date(prevService.time.visited).getTime() + prevService.time.duration * 60000)
          ) / (60 * 1000) // minutes
  ...
  const unscheduledGroups = {
    noise: { label: 'Noise Points (cluster -1)', services: originalClusters.noise },
    outliers: { label: 'Outliers (cluster -2)', services: originalClusters.outliers },
    overlap: { label: 'Would Overlap (cluster -3)', services: originalClusters.overlap },
    outOfRange: { label: 'Out of Range (cluster -4)', services: originalClusters.outOfRange },
...
function calculateClusterStats(services) {
  ...
  const duration = (
    new Date(last.time.visited).getTime() + last.time.duration * 60000 - 
