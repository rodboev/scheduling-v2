import { calculateTravelTime } from '@/app/map/utils/distance'
    ...
    while (schedulableServices.length > 0) {
      ...
      while (schedulableServices.length > 0) {
        ...
        for (const service of schedulableServices) {
          ...
          const distance = await getServiceDistance(service, lastScheduled)
...
// Use distance matrix if provided, otherwise fetch from API
const getServiceDistance = async (from, to) => {
  if (distanceMatrix) {
    const fromIndex = services.findIndex(s => s.id === from.id)
    const toIndex = services.findIndex(s => s.id === to.id)
